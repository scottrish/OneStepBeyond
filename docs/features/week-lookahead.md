...
- Lists the next seven days, each showing: the date (tap to jump straight
  into planning that day), a qualitative capacity phrase, anything due
  that day (linked to its Assignment), that day's Activities, and any
  already-planned sessions (with completion state and a remove action for
  not-yet-done sessions).
